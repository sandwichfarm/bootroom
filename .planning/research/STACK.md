...
**Domain:** Rust CLI + embedded HTTP server + browser-served qemu-wasm test harness for kernels
**Researched:** 2026-05-17
**Confidence:** HIGH for Rust ecosystem, HIGH for qemu-wasm integration mechanics, MEDIUM for headless-CI strategy (requires a spike to confirm Playwright + COOP/COEP + xterm-pty bridge works end-to-end)

---

## Executive Summary

bootroom is a single static Rust binary that:

1. Embeds the qemu-wasm submodule's emscripten output (`*.js`, `*.wasm`, `*.worker.js`, `*.data`) and a vanilla-JS UI into the executable via `include_dir!`.
2. Serves them over **axum** with `Cross-Origin-Opener-Policy: same-origin` and `Cross-Origin-Embedder-Policy: require-corp` (mandatory — qemu-wasm uses `SharedArrayBuffer` + pthreads).
3. Bridges the browser UI to the running guest by writing bytes into the xterm-pty **slave** PTY (`slave.write(bytes)`). That is the only injection surface qemu-wasm exposes today, and it's the one its own demo uses.
4. Streams config to the page via a single JSON HTTP `GET /api/config` (TOML on disk → JSON on the wire); pushes kernel-reload events via **Server-Sent Events** (`GET /api/events`); pushes button-press commands the *other* direction via plain `POST /api/action` (no need for full-duplex WebSocket — the UI runs the qemu side, the server only needs to nudge it).
5. Watches the kernel artifact path with **notify-debouncer-full** and broadcasts a `kernel-changed` SSE event so the UI can re-load.
6. For CI, drives a headless **Chromium via `chromiumoxide`** (or Playwright as an external dep; both verified). qemu-wasm cannot run outside a browser — it relies on `WebAssembly.Module`/`Instance` for dynamic TCG JIT compilation, which WASI runtimes don't provide.
7. Ships via **`cargo-dist`** (prebuilt GitHub Release binaries for x86_64-linux, aarch64-linux, x86_64-darwin, aarch64-darwin) + `cargo install bootroom` for the source-builders. `cargo-binstall` auto-discovers cargo-dist artifacts, so users get fast installs for free.
...
> The qemu-wasm demo currently loads xterm via `https://unpkg.com/xterm@5.3.0/lib/xterm.js`. **Vendor those files** into bootroom's embedded `web/` directory — a CI-facing dev tool must not rely on a CDN.
...
Read this section once; it's the load-bearing answer the roadmap will depend on.

### What qemu-wasm ships (per `qemu-wasm/examples/riscv64/src/htdocs/`)

After running the docker build in `qemu-wasm/README.md`, you get five files that bootroom must serve:

1. `qemu-system-riscv64` → renamed to `out.js` (emscripten loader)
2. `qemu-system-riscv64.wasm` — the QEMU wasm module
3. `qemu-system-riscv64.worker.js` — pthread worker shim
4. `qemu-system-riscv64.data` — preload pack (BIOS, OpenSBI firmware, **kernel image**, rootfs)
5. `load.js` — generated by emscripten's `file_packager.py` to register the data file

Plus the page's own `index.html` and `module.js` (QEMU command-line arguments).
...
This is the headline UX. Reference implementation:

1. User runs `make` in the kernel repo → kernel artifact's mtime changes.
2. `notify-debouncer-full` (with ~250 ms window) fires one event on bootroom's tokio task.
3. bootroom **re-runs `emscripten/tools/file_packager.py`** to produce a fresh `*.data` + `load.js`. **Open question for the roadmap:** can we skip `file_packager.py` and instead inject the kernel as a separate fetch-then-`FS.writeFile` call inside the browser? That avoids invoking emscripten at runtime. Likely YES — emscripten's `FS.writeFile` is exposed via `Module.FS`, and the `-kernel /pack/Image` argv can point at any path we pre-populate. Spike in Phase 1.
4. bootroom publishes a `kernel-changed` event on its `tokio::broadcast::Sender`.
5. Every connected browser tab's `EventSource` receives the event. The UI shows a "Reload" toast; clicking it (or "Launch") tears down the current iframe / Worker and re-instantiates the emscripten module with the new data.
6. Because qemu-wasm is loaded via an ES module `import('./out.js')`, full teardown = drop references + re-call `initEmscriptenModule(Module)`. Cleanest implementation is a fresh `<iframe>` (or new Worker), since emscripten doesn't expose a clean shutdown path. **Don't** try to GC the existing module — emscripten doesn't support it (per emscripten issue #18080).
...
User clicks button
   ↓
[browser] POST /api/action {id: "boot-then-ls"}
   ↓ (round-trip back through the server is unnecessary for local-only events)
[browser, alternative path] looks up action by id in already-loaded config
   ↓
[browser] for each step: ptySlave.write(stringToBytes(step.payload))
   ↓
QEMU's chardev sees bytes on its emulated UART → guest kernel sees stdin
```

**Recommendation:** keep the action lookup **entirely client-side** (config already shipped via `GET /api/config` at page load). The server's `POST /api/action` exists only so CI/scripts can drive the harness over HTTP without needing the UI. This keeps the live-UI fast and the CI path identical.

### Capturing serial output for assertions

`xterm-pty`'s `slave.onReadable(() => { const bytes = slave.read(); … })` gives the browser-side view of *everything* the guest writes to its serial console. To get it to the server for CI:

1. Browser opens a WebSocket to `/api/serial-tap` (one-way *upstream*, so technically SSE-from-client would work, but no such API exists — WebSocket is the right tool here).
2. Every `onReadable` callback drains the slave and forwards bytes to the server.
3. The server appends to a per-session ring buffer.
4. `bootroom run` matches assertions (`contains: "PASS"`, `regex: "^OK \\d+ tests"`) against that buffer with a timeout.

This is the **one** place a WebSocket is genuinely better than SSE: client → server byte stream. Everywhere else (config push, kernel-changed notifications) SSE is simpler.

### Headless CI mode

qemu-wasm **cannot** run in a standalone WASI runtime (wasmtime, wasmer, Node.js's `WebAssembly.instantiate`). It requires:
...
So `bootroom run` must drive a real browser. Concrete plan:

1. **chromiumoxide** launches headless Chromium with `--headless=new --enable-features=SharedArrayBuffer`.
2. Navigates to the same `http://127.0.0.1:<port>/` the dev server serves.
3. Listens to the WebSocket-tapped serial buffer server-side (no CDP needed for assertions — it's bootroom's own server).
4. Drives buttons via `POST /api/action` (no DOM clicking needed; chromiumoxide is only there to provide the browser environment qemu-wasm requires).
5. Exits 0 on assertion match, 1 on timeout/mismatch.

This is conceptually simpler than the alternative (Playwright over an MCP layer or shell-out); chromiumoxide stays inside the Rust binary's dependency tree.

**Open risk:** Chromium's headless mode has had on-again, off-again issues with SharedArrayBuffer + `--headless=old`. Use `--headless=new` (the default since Chrome 112). Confirm in a Phase 1 spike. If it doesn't work, fall back to Playwright invoked as a subprocess (`bootroom run` can shell out to `npx playwright` or a vendored binary); this drops the all-Rust-dependency-tree property but is well-trodden.
...
repository = "https://github.com/<owner>/bootroom"
description = "Web-based test harness for RISC-V kernels via qemu-wasm"

[dependencies]
# Async runtime + HTTP
tokio = { version = "1", features = ["rt-multi-thread", "macros", "signal", "sync", "fs", "process"] }
axum = { version = "0.8", features = ["ws", "macros"] }
tower = "0.5"
tower-http = { version = "0.6", features = ["set-header", "trace", "fs", "cors"] }
futures-util = "0.3"
...
Pin exact versions in `Cargo.lock` only; let `Cargo.toml` use `^` ranges so dependabot/renovate can keep them current. Confidence on versions: **HIGH** (verified via crates.io listings in 2026-02 to 2026-03 search results). Run `cargo update` once and pin from there.
...
│   ├── cli.rs               # CliArgs + Subcommand enum (derive)
│   ├── config.rs            # TOML schema (Config, Action, Group, Scenario)
│   ├── server.rs            # axum app builder, routes, COOP/COEP layer
│   ├── routes/
│   │   ├── config_api.rs    # GET /api/config → JSON
│   │   ├── action.rs        # POST /api/action
│   │   ├── events.rs        # GET /api/events (SSE)
│   │   ├── serial_tap.rs    # WS /api/serial-tap
│   │   └── assets.rs        # static handler over include_dir
│   ├── watcher.rs           # notify-debouncer-full → broadcast::Sender
│   ├── headless.rs          # `bootroom run` chromiumoxide driver
│   └── qemu_assets.rs       # include_dir!("../qemu-wasm/examples/riscv64/build")
├── web/                     # embedded into binary via include_dir!
│   ├── index.html
...
│       ├── xterm.css
│       └── xterm-pty.js
├── qemu-wasm/               # git submodule (existing)
├── bootroom.example.toml
└── README.md
```

The `include_dir!` macros at the top of `qemu_assets.rs` and `assets.rs` embed both `web/` and the built qemu-wasm output. **Build script note:** add a `build.rs` (or a `Makefile` target) that runs the docker-based qemu-wasm build once and caches its output in a path `include_dir!` references. Don't try to build qemu-wasm from `build.rs` on every `cargo build` — it's a multi-minute docker compilation. Gate behind an explicit `make qemu-assets` target.
...
**If headless-CI Chromium + SharedArrayBuffer turns out flaky:**
...
**If qemu-wasm assets exceed ~30 MB and binary size matters:**
...
**If you want to add a Firefox-tests CI matrix later:**
- Replace chromiumoxide with **fantoccini** (WebDriver), drives geckodriver + Firefox in addition to Chrome.

**If the kernel reload flow needs to be sub-second:**
...
**If you ever want non-RISC-V architectures (currently out of scope per PROJECT.md):**
...
*Stack research for: bootroom (Rust CLI + qemu-wasm-in-browser kernel test harness)*
*Researched: 2026-05-17*
